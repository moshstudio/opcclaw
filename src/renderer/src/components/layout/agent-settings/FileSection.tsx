      ...
      <div className="space-y-4 pt-1">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 px-1">
          <div className="space-y-1 min-w-0 flex-1">
            <h3 className="text-[11px] font-black uppercase tracking-wider text-foreground/80">
              {t('common.bootstrap_files')}
            </h3>
            <p className="text-[9px] text-muted-foreground/60 leading-relaxed max-w-[400px]">
              {t('common.bootstrap_desc')}
            </p>
            <p className="text-[8px] text-muted-foreground/30 font-mono mt-1 truncate group-hover:text-muted-foreground/60 transition-colors">
        ...
        <div className="flex flex-col min-h-[450px] border border-border/40 rounded-2xl bg-muted/5 overflow-hidden shadow-inner group/editor">
          {/* 顶部集成工具栏 */}
          <div className="flex w-[260px] overflow-x-auto items-center justify-between gap-3 px-3 py-2 border-b border-border/40 bg-background/30 backdrop-blur-sm">
            <div className="flex items-center flex-1 min-w-0 gap-2">
              ...
              >
                <SelectTrigger className="h-8 px-3 text-[11px]  bg-background border border-border/30 shadow-sm hover:bg-muted/50 hover:border-border/50 transition-all focus:ring-1 focus:ring-primary/20 w-full rounded-lg overflow-hidden">
                ...
                <SelectContent className="min-w-[220px] rounded-xl border border-border/40 shadow-xl">
                  {files.map((file) => (
                    <SelectItem key={file.name} value={file.name} className="text-[11px]  py-2.5">
                      <div className="flex items-center justify-between w-full gap-4">
                        <span className="truncate">{file.name}</span>
                        {file.missing && (
                          <span className="text-[7px] font-bold uppercase text-destructive/80 bg-destructive/10 px-1 py-0.5 rounded border border-destructive/20 ml-2">
                            {t('common.missing')}
                          </span>
                        )}
                      </div>
          ...
          <div className="relative flex-1 flex flex-col">
            ...
            <Textarea
              ...
              className="flex-1 w-full min-h-[380px] text-[10px]  bg-transparent border-none rounded-none resize-none focus-visible:ring-0 p-5 pt-9 pb-10 leading-relaxed custom-scrollbar"
            />
          </div>
        </div>
