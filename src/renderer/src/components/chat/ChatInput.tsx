      ...
      <AnimatePresence>
        {currentError && chatStatus === 'error' && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="px-4 py-2 bg-destructive/10 text-destructive text-xs text-center border-t border-destructive/20 font-sans font-bold max-w-full overflow-hidden text-ellipsis shadow-[0_-4px_12px_rgba(220,38,38,0.1)]"
          >
            {currentError}
          </motion.div>
        )}
      ...
      <div className="bg-background/50 backdrop-blur-sm border-t p-4 shrink-0 transition-all duration-300 w-full min-w-0">
        <div className="max-w-4xl mx-auto w-full min-w-0 relative">
          <Sender
            ...
            className="rounded-2xl shadow-sm border-border/60 font-sans"
