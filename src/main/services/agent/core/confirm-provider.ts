...
export class ConfirmProvider {
   ...
   * 确认是否安装运行环境
   */
  public static async confirmInstallEnv(ctx: any, env: string): Promise<boolean> {
    return this.run(ctx, {
      key: `env:install:${env}`,
   ...
   * 确认是否开启 Host 模式浏览器
   */
  public static async confirmOpenHostBrowser(ctx: any): Promise<boolean> {
    return this.run(ctx, {
  ...
  private static async run(ctx: any, task: ConfirmTask): Promise<boolean> {
    ...
    if (remembered !== undefined) {
      logger.info(`[Auto-Confirm] Bypassing UI for ${task.key}: ${remembered}`)
      return remembered
    }
