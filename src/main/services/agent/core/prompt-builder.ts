...
export class AgentPromptBuilder {
  ...
  private buildToolingSection(tools: Tool[]): string[] {
    ...
    return [
      ...
      ...toolLines,
      '### 工具使用原则',
      '- **精确调用**：工具名称区分大小写，请严格按照列表中的名称进行调用。',
      '- **充分利用**：请主动、充分且合理地利用以上提供给您的各类工具来解决问题。',
      '- **主动搜索与探索**：在遇到不清楚的信息、缺乏特定知识或不确定的用法时，如果工具箱中包含浏览器或搜索相关的工具（可用的话），务必优先使用浏览器工具上网查找和获取准确信息，不要盲目猜测。',
      ''
    ]
  }
  ...
  private async buildSkillsSection(): Promise<string[]> {
    ...
    return [
      ...
      '- **加载与执行**：如果某个技能完全契合当前任务，请先使用 `read` 读取其 `SKILL.md` 指令文件，并严格执行其中的规则。',
      '- **技能来源与属性**：',
      '  - `built-in`：系统提供的核心能力，不可直接修改，作为所有操作的基石。',
      '  - `managed`：用户安装的全局技能，跨智能体共享。',
      '  - `workspace`：当前智能体私有的工作区技能，优先级最高，用于针对特定项目进行定制。',
      '- **技能存储与创建**：你可以创建或更新技能。',
      '  - **智能体专用技能**：存放在当前工作区的 `skills/` 目录下（例如 `skills/my-skill/SKILL.md`）。这是默认且推荐的位置。',
      `  - **全局/跨智能体技能**：存放在显式授权的目录中（参考 <available_skills> 中的路径）。`,
      '- **技能架构**：每个技能都是一个文件夹，必须包含 `SKILL.md` 文件（带 YAML frontmatter）。你可以在该目录下创建 `scripts/`、`references/` 等子目录。',
      skillsPrompt,
      ''
    ]
  }
